...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const textchannel = interaction.guild.channels.cache.get('1037573414413684827');
        const memberArray = textchannel.members.map(member => member.user.username);
        if (!memberArray.includes(killed)) {
            await interaction.reply(`Agent, please stay focused. You just killed a civilian.`);
            return;
        }
        ...
        await interaction.reply(`You have successfully reported you killed **${killed}**. You now have ${killer_score['score']} points. Watch your back, agent.`);
    },
};
